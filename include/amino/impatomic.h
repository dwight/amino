...
CPP0X(private:)
    ...
    #include "atomic_base/x86.h"
#elif defined(PPC) && defined(AIX)
    #include "atomic_base/aixppc.h"
#else
...
typedef struct atomic_address
{
    ...
    friend void atomic_store_explicit( volatile atomic_address*, void*,
                                       memory_order );
    friend void* atomic_load_explicit( const volatile atomic_address* const, memory_order );
...
CPP0X(private:)
#endif
    volatile unsigned int __f__;
} ALIGN_DEF atomic_uint;
...
CPP0X(private:)
#endif
    volatile unsigned long __f__;
} ALIGN_DEF atomic_ulong;
...
CPP0X(private:)
#endif
    volatile long long __f__;
}ALIGN_DEF atomic_llong;
...
CPP0X(private:)
#endif
    volatile unsigned long long __f__;
}ALIGN_DEF atomic_ullong;
...
CPP0X(private:)
#endif
    volatile wchar_t __f__;
}ALIGN_DEF atomic_wchar_t;
...
{
#ifdef PPC
    return _ATOMIC_LOAD_( __a__, __x__ ) != 0;
#else
...
#ifdef PPC
    int tmp = __m__;
    _ATOMIC_STORE_( __a__, tmp, __x__ );
#else
...
#ifdef PPC
    int tmp = __m__;
    return _ATOMIC_MODIFY_( __a__, ops_swap, tmp, __x__ );
#else
    return _ATOMIC_MODIFY_( __a__, ops_swap, __m__, __x__ ); 
#endif
}
  ...
  memory_order __x__, memory_order __y__ )
{
    ...
    int tmp = __m__;
    int tmp1 = *__e__;
    bool result = _ATOMIC_CMPSWP_( __a__, &tmp1, tmp, __x__ );
    *__e__ = !(tmp1==0);
    return result;
#else
...
( const volatile atomic_address* const __a__, memory_order __x__ )
{ return _ATOMIC_LOAD_((volatile atomic_address * ) __a__, __x__ ); }
...
{ 
    void * old;
    do{
    old = __a__->load(memory_order_relaxed);
    }while(__a__->compare_swap(old, __m__));
    return old;
}
...
{ 
#ifdef PPC
	char tmp =  _ATOMIC_LOAD_( __a__, __x__ ); 
	return tmp; 
#else
	return _ATOMIC_LOAD_( __a__, __x__ ); 
#endif
...
{ 
        ...
        int tmp = __m__;
	int tmp1 = *__e__;
	bool ret =  _ATOMIC_CMPSWP_( __a__, &tmp1, tmp, __x__ );	
        *__e__ = tmp1;
	return ret;
#else
	return _ATOMIC_CMPSWP_( __a__, __e__, __m__, __x__ ); 
...
{
#ifdef PPC
	signed char tmp =  _ATOMIC_LOAD_( __a__, __x__ ); 
	return tmp; 
#else
	return _ATOMIC_LOAD_( __a__, __x__ ); 
#endif
...
{
#ifdef PPC
	unsigned char tmp =  _ATOMIC_LOAD_( __a__, __x__ ); 
	return tmp; 
#else
	return _ATOMIC_LOAD_( __a__, __x__ ); 
#endif
...
{
#ifdef PPC
	short tmp =  _ATOMIC_LOAD_( __a__, __x__ ); 
	return tmp; 
#else
	return _ATOMIC_LOAD_( __a__, __x__ ); 
#endif
...
{
#ifdef PPC
	unsigned short tmp =  _ATOMIC_LOAD_( __a__, __x__ ); 
	return tmp; 
#else
	return _ATOMIC_LOAD_( __a__, __x__ ); 
#endif
...
_ATOMIC_MODIFY_( __a__, ops_swap, __m__, memory_order_seq_cst )

#define atomic_swap_explicit( __a__, __m__, __x__ ) \
_ATOMIC_MODIFY_( __a__, ops_swap, __m__, __x__ )
...
/* gcc-x86 builtins */
#if defined(GCC) && defined(X86)
#include <amino/arch/x86/gcc_builtins.h>
#elif defined(XLC) && defined(PPC)
#include <amino/arch/ppc/xlc_builtins.h>
#else
#include <amino/arch/compiler.h>
#endif
...
inline void* atomic_address::load( memory_order __x__ ) const volatile
{
...
inline int atomic_int::load( memory_order __x__ ) volatile
{
    if(__x__ == memory_order_relaxed)
        return __f__;
    return atomic_load_explicit( this, __x__ );
...
{ return _ATOMIC_MODIFY_( this, ops_swap, __v__, __x__ ); }
...
template< typename T >
T* atomic<T*>::load( memory_order __x__ ) const volatile
